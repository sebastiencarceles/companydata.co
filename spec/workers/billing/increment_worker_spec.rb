...
RSpec.describe Billing::IncrementWorker, type: :worker do
  ...
  # TODO temporarily unplug email notifications
  ...
  #   expect(Sidekiq::Worker.jobs.last["args"][0]["arguments"]).to eq ["UserMailer", "no_subscription", "deliver_now", { "user_id" => user.id, "_aj_symbol_keys" => ["user_id"] }]
  # end
  ...
  #   expect { subject }.to change { Sidekiq::Worker.jobs.size }.by(1)
  #   expect(Sidekiq::Worker.jobs.last["args"][0]["arguments"]).to eq ["UserMailer", "missing_payment_method", "deliver_now", { "user_id" => user.id, "_aj_symbol_keys" => ["user_id"] }]
  # end
  ...
  #   expect { subject }.to change { Sidekiq::Worker.jobs.size }.by(1)
  #   expect(Sidekiq::Worker.jobs.last["args"][0]["arguments"]).to eq ["UserMailer", "unpaid_invoices", "deliver_now", { "user_id" => user.id, "_aj_symbol_keys" => ["user_id"] }]
  # end
end
